    ...
    \begin{column}{.55\linewidth}
      \begin{center}
        \begin{tikzpicture}
          ...
          \draw[con,hide,only={<3->{show}},dashed] (mcxsec)--(xsec) node[midway,above]{?};
          \draw[con,hide,only={<3->{show}},dashed] (mcpdfs)--(pdfi) node[pos=.35,above]{?};
          \draw[con,hide,only={<3->{show}},dashed] (mchvyi)--(hvyi) node[pos=.35,above]{?};
          \draw[con,hide,only={<3->{show}},dashed] (mccoll)--(hep);
        \end{tikzpicture}
      \end{center}
      \vspace*{1ex} \uncover<3->{
        %% Linter wants space before parenthesis - may be entirely
        %% wrong here. 
        \tikz[baseline={ (0,-.1)}]{\draw[con,dashed] (0,0) to (.4,0);} not
        available in \texttt{o2-aod-mc-producer-workflow}, but
        somewhat with \texttt{o2-sim-mctracks-to-aod}.\newline%
...
\end{verbatim}
  \end{exampleblock}
  \uncover<2->{
    {\footnotesize{} (BTW, it should be \texttt{--noDis{\itshape{}
          k\/}Output}), see
...
\begin{frame}
  ...
  \uncover<+->{\alert{\href{https://github.com/AliceO2Group/AliceO2/pull/12032}{Merge
        request \# 12032} changes that}}
...
\begin{frame}
  ...
  \uncover<+->{\alert{\href{https://github.com/AliceO2Group/AliceO2/pull/12039}{Merge
        request \# 12039} changes that}}
